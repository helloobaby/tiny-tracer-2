...
typedef VOID AntiDBGCallBack(const ADDRINT Address, const CHAR* name,
                             uint32_t argCount, VOID* arg1, VOID* arg2,
...
VOID LogAntiDbg(const WatchedType wType, const ADDRINT Address, const char* msg,
...
VOID AntiDbg::WatchMemoryAccess(ADDRINT addr, UINT32 size,
                                const ADDRINT insAddr) {
  PinLocker locker;
  ...
  if (addr == 0x7ffe02d4) {
    return LogAntiDbg(wType, insAddr, "KUSER_SHARED_DATA accessed",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "debug-flags.html#kuser_shared_data");
  }
#ifdef _WIN64
  ...
  if (addr == heapFlags || addr == heapForceFlags) {
    return LogAntiDbg(wType, insAddr, "Heap Flags accessed",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "debug-flags.html#manual-checks-heap-flags");
  }
#else
...
VOID AntiDbg::WatchCompareSoftBrk(ADDRINT Address, UINT64 immVal) {
  ...
  if (isSet) {
    LogAntiDbg(wType, Address, "Software Breakpoint comparison",
               "https://anti-debug.checkpoint.com/techniques/"
               "process-memory.html#anti-step-over");
  }
}
...
VOID AntiDbg::FlagsCheck(const CONTEXT* ctxt, THREADID tid) {
  ...
  const ADDRINT* stackPtr =
      reinterpret_cast<ADDRINT*>(PIN_GetContextReg(ctxt, REG_STACK_PTR));
  ...
  LogAntiDbg(wType, Address, "Trap Flag set",
             "https://anti-debug.checkpoint.com/techniques/"
             "assembly.html#popf_and_trap_flag");
...
VOID AntiDbg::FlagsCheck_after(const CONTEXT* ctxt, THREADID tid, ADDRINT eip) {
  ...
  EXCEPTION_INFO exc;
  PIN_InitWindowsExceptionInfo(&exc, 0x80000004L,
                               eip);  // NTSTATUS_STATUS_SINGLE_STEP
...
VOID AntiDbg::InterruptCheck(CONTEXT* ctxt) {
    ...
    const ADDRINT Address = PIN_GetContextReg(ctxt, REG_INST_PTR);
    {
        ...
        else if (interruptID == 0x2E) {
            LogAntiDbg(
                wType, Address, "INT2E",
                "If Host is x64 , but app is x86 , INT2E is AntiDebug trick");
        }
...
VOID AntiDbgLogFuncOccurrence(const ADDRINT Address, const CHAR* name,
                              uint32_t argCount, VOID* arg1, VOID* arg2,
                              VOID* arg3, VOID* arg4, VOID* arg5) {
  PinLocker locker;
  ...
  if (itr != funcToLink.end()) {
    return LogAntiDbg(wType, Address, ss.str().c_str(), itr->second.c_str());
  }
  return LogAntiDbg(wType, Address, ss.str().c_str());
}

VOID AntiDbg_LoadLibrary(const ADDRINT Address, const CHAR* name,
...
VOID AntiDbg_BlockInput(const ADDRINT Address, const CHAR* name,
                        uint32_t argCount, VOID* arg1, VOID* arg2, VOID* arg3,
                        VOID* arg4, VOID* arg5) {
  if (!argCount) return;
  ...
  if (BlockInputOccurrences > 1) {
    return LogAntiDbg(wType, Address, "^ user32!BlockInput()",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "interactive.html#blockinput");
  }
}

VOID AntiDbg_NtSetInformationThread(const ADDRINT Address, const CHAR* name,
                                    uint32_t argCount, VOID* arg1, VOID* arg2,
                                    VOID* arg3, VOID* arg4, VOID* arg5) {
  if (!argCount) return;
  ...
  // Check if NtSetInformationThread has been called with parameter
  ...
  if (int((size_t)arg1) == NtCurrentThread &&
    ...
    return LogAntiDbg(wType, Address, "^ ntdll!NtSetInformationThread()",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "interactive.html#ntsetinformationthread");
  }
}

VOID AntiDbg_RaiseException(const ADDRINT Address, const CHAR* name,
                            uint32_t argCount, VOID* arg1, VOID* arg2,
                            VOID* arg3, VOID* arg4, VOID* arg5) {
  if (!argCount) return;
  ...
  enum ExceptionCode {
    kDBG_CONTROL_C = 0x40010005,
    kDBG_RIPEVENT = 0x40010007
  };
  // kernel32!RaiseException() with DBG_CONTROL_C or DBG_RIPEVENT
  if (int((size_t)arg1) == ExceptionCode::kDBG_CONTROL_C ||
      int((size_t)arg1) == ExceptionCode::kDBG_RIPEVENT) {
    return LogAntiDbg(wType, Address, "^ kernel32!RaiseException()",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "exceptions.html#raiseexception");
  }
}

VOID AntiDbg_NtQuerySystemInformation(const ADDRINT Address, const CHAR* name,
                                      uint32_t argCount, VOID* arg1, VOID* arg2,
                                      VOID* arg3, VOID* arg4, VOID* arg5) {
  if (!argCount) return;
  ...
  // function ntdll!NtQuerySystemInformation() with first parameter set to 0x23
  // (SystemKernelDebuggerInformation)
  if (int((size_t)arg1) ==
      SystemInformationClass::SystemKernelDebuggerInformation) {
    return LogAntiDbg(
        wType, Address,
        "^ ntdll!NtQuerySystemInformation (SystemKernelDebuggerInformation)",
        "https://anti-debug.checkpoint.com/techniques/"
        "debug-flags.html#using-win32-api-checks-ntquerysysteminformation");
  }
}

VOID AntiDbg_NtQueryInformationProcess(const ADDRINT Address, const CHAR* name,
                                       uint32_t argCount, VOID* arg1,
  ...
  // function ntdll!NtQueryInformationProcess with ProcessInformationClass == 7
  // (ProcessDebugPort)
  if (int((size_t)arg2) == ProcessInformationClass::ProcessDebugPort) {
    return LogAntiDbg(wType, Address,
                      "^ ntdll!NtQueryInformationProcess (ProcessDebugPort)",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "debug-flags.html#using-win32-api-"
                      "ntqueryinformationprocess-processdebugport");
  }
  // function ntdll!NtQueryInformationProcess with ProcessInformationClass ==
  // 0x1f (ProcessDebugFlags)
  if (int((size_t)arg2) == ProcessInformationClass::ProcessDebugFlags) {
    return LogAntiDbg(wType, Address,
                      "^ ntdll!NtQueryInformationProcess (ProcessDebugFlags)",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "debug-flags.html#using-win32-api-"
  ...
  if (int((size_t)arg2) == ProcessInformationClass::ProcessDebugObjectHandle) {
    return LogAntiDbg(
        wType, Address,
        "^ ntdll!NtQueryInformationProcess (ProcessDebugObjectHandle)",
        "https://anti-debug.checkpoint.com/techniques/"
        "debug-flags.html#using-win32-api-ntqueryinformationprocess-"
        "processdebugobjecthandle");
  }
}

VOID AntiDbg_NtQueryObject(const ADDRINT Address, const CHAR* name,
                           uint32_t argCount, VOID* arg1, VOID* arg2,
                           VOID* arg3, VOID* arg4, VOID* arg5) {
  if (argCount < 2) return;
  ...
  // ntdll!NtQueryObject() to access DebugObject (with ObjectAllTypesInformation
  // as 2nd argument)
  if (int((size_t)arg2) == ObjectInformationClass::ObjectAllTypesInformation) {
    return LogAntiDbg(wType, Address,
                      "^ ntdll!NtQueryObject (ObjectAllTypesInformation)",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "object-handles.html#ntqueryobject");
  }
}

VOID AntiDbg_CreateFile(const ADDRINT Address, const CHAR* name,
                        uint32_t argCount, VOID* arg1, VOID* arg2, VOID* arg3,
                        VOID* arg4, VOID* arg5) {
  ...
  // kernel32!CreateFileX called on the module itself with Exclusive access, or
  // on loaded libraries Check only exclusive accesses for optimization
  if (int((size_t)arg3) == 0) {
    ...
    if (util::isStrEqualI(_argStr, moduleName)) {
      return LogAntiDbg(wType, Address, "^ kernel32!CreateFile on module",
                        "https://anti-debug.checkpoint.com/techniques/"
                        "object-handles.html#createfile");
    }
    ...
    for (size_t i = 0; i < loadedLib.size(); i++) {
      if (util::isStrEqualI(_argStr, loadedLib[i])) {
        return LogAntiDbg(wType, Address, "^ kernel32!CreateFile on loaded lib",
                          "https://anti-debug.checkpoint.com/techniques/"
                          "object-handles.html#loadlibrary");
      }
    }
  }
}
...
BOOL getPEB(CONTEXT* ctxt, ADDRINT& pebAddr) {
  BOOL is_ok = FALSE;
#ifdef _WIN64
  // Read the value from the memory address pointed by GS:[60h] and save it in
  // the global variable
  ADDRINT gsValue;
  PIN_GetContextRegval(ctxt, REG_SEG_GS_BASE,
                       reinterpret_cast<UINT8*>(&gsValue));
  gsValue += 0x60;
  // Save PEB Address
  if (PIN_SafeCopy(&pebAddr, reinterpret_cast<VOID*>(gsValue), sizeof(pebAddr)))
    is_ok = TRUE;
#else
  // Read the value from the memory address pointed by FS:[30h] and save it in
  // the global variable
  ADDRINT fsValue;
  PIN_GetContextRegval(ctxt, REG_SEG_FS_BASE,
                       reinterpret_cast<UINT8*>(&fsValue));
  fsValue += 0x30;

  // Save PEB Address
  if (PIN_SafeCopy(&pebAddr, reinterpret_cast<VOID*>(fsValue), sizeof(pebAddr)))
    is_ok = TRUE;
#endif
  return is_ok;
}

VOID AntiDbg::WatchThreadStart(THREADID threadid, CONTEXT* ctxt, INT32 flags,
                               VOID* v) {
  PinLocker locker;
  ...
  // Get Heap flags addresses
  // (https://anti-debug.checkpoint.com/techniques/debug-flags.html#manual-checks-heap-flags)
  ADDRINT heapBase;
  PIN_SafeCopy(&heapBase, reinterpret_cast<VOID*>(pebAddr + 0x30),
               sizeof(heapBase));
  ADDRINT heapFlagsOffset = WinIsWindowsVistaOrGreater() ? 0x70 : 0x14;
  ADDRINT heapForceFlagsOffset = WinIsWindowsVistaOrGreater() ? 0x74 : 0x18;
  ...
  // (https://anti-debug.checkpoint.com/techniques/debug-flags.html#manual-checks-heap-flags)
  ADDRINT heapBase;
  ADDRINT heapBaseOffset = WinIsNativeOs32() ? 0x18 : 0x1030;
  PIN_SafeCopy(&heapBase, reinterpret_cast<VOID*>(pebAddr + heapBaseOffset),
               sizeof(heapBase));
  ADDRINT heapFlagsOffset = WinIsWindowsVistaOrGreater() ? 0x40 : 0x0C;
  ADDRINT heapForceFlagsOffset = WinIsWindowsVistaOrGreater() ? 0x44 : 0x10;
  heapFlags = heapBase + heapFlagsOffset;
  heapForceFlags = heapBase + heapForceFlagsOffset;
#endif
}
...
VOID AntiDbg_After_CloseHandle(ADDRINT Address, ADDRINT result) {
  ...
  if (!result) {
    // Invalid closure
    return LogAntiDbg(wType, Address,
                      "^ kernel32!CloseHandle (INVALID_HNDL_VAL)",
                      "https://anti-debug.checkpoint.com/techniques/"
                      "object-handles.html#closehandle");
  }
}
...
bool AntiDbgAddCallbackBefore(IMG Image, char* fName, uint32_t argNum,
  ...
  if (RTN_Valid(funcRtn)) {
    ...
    RTN_InsertCall(
        funcRtn, IPOINT_BEFORE, AFUNPTR(callback), IARG_RETURN_IP, IARG_ADDRINT,
        fName, IARG_UINT32, argNum, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
...
VOID AntiDbg::MonitorAntiDbgFunctions(IMG Image) {
  funcToLink["IsDebuggerPresent"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "debug-flags.html#using-win32-api-isdebuggerpresent";
  funcToLink["CheckRemoteDebuggerPresent"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "debug-flags.html#using-win32-api-checkremotedebuggerpresent";
  funcToLink["RtlQueryProcessHeapInformation"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "debug-flags.html#using-win32-api-checks-rtlqueryprocessheapinformation";
  funcToLink["RtlQueryProcessDebugInformation"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "debug-flags.html#using-win32-api-checks-rtlqueryprocessdebuginformation";
  funcToLink["HeapWalk"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "debug-flags.html#manual-checks-heap-protection";
  funcToLink["CsrGetProcessId"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "object-handles.html#openprocess";
  funcToLink["SetUnhandledExceptionFilter"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "exceptions.html#unhandledexceptionfilter";
  funcToLink["RaiseException"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "exceptions.html#raiseexception";
  funcToLink["DebugActiveProcess"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#self-debugging";
  funcToLink["DbgUiDebugActiveProcess"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#self-debugging";
  funcToLink["NtDebugActiveProcess"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#self-debugging";
  funcToLink["GenerateConsoleCtrlEvent"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#generateconsolectrlevent";
  funcToLink["GetWindowTextA"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#suspendthread";
  funcToLink["GetWindowTextW"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#suspendthread";
  funcToLink["SwitchDesktop"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#switchdesktop";
  funcToLink["OutputDebugStringA"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#outputdebugstring";
  funcToLink["OutputDebugStringW"] =
      "https://anti-debug.checkpoint.com/techniques/"
      "interactive.html#outputdebugstring";
  ...
  if (util::iequals(dllName, "ntdll")) {
    AntiDbgAddCallbackBefore(Image, "CsrGetProcessId", 0,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "RtlQueryProcessHeapInformation", 1,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "RtlQueryProcessDebugInformation", 3,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "NtQueryInformationProcess", 5,
                             AntiDbg_NtQueryInformationProcess);
    AntiDbgAddCallbackBefore(Image, "NtQuerySystemInformation", 4,
                             AntiDbg_NtQuerySystemInformation);
    AntiDbgAddCallbackBefore(Image, "DbgUiDebugActiveProcess", 1,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "NtDebugActiveProcess", 2,
                             AntiDbgLogFuncOccurrence);
    ...
    if (m_Settings.antidebug >= ANTIDEBUG_DEEP) {
      AntiDbgAddCallbackBefore(Image, "NtQueryObject", 5,
                               AntiDbg_NtQueryObject);
    }
  }
  if (util::iequals(dllName, "kernel32")) {
    ...
    AntiDbgAddCallbackBefore(Image, "IsDebuggerPresent", 0,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "CheckRemoteDebuggerPresent", 2,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "HeapWalk", 2, AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "SetUnhandledExceptionFilter", 1,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "RaiseException", 4,
                             AntiDbg_RaiseException);
    AntiDbgAddCallbackBefore(Image, "DebugActiveProcess", 1,
                             AntiDbgLogFuncOccurrence);
    AntiDbgAddCallbackBefore(Image, "GenerateConsoleCtrlEvent", 2,
                             AntiDbgLogFuncOccurrence);
    ...
    ////////////////////////////////////
    if (m_Settings.antidebug >= ANTIDEBUG_DEEP) {
      AntiDbgAddCallbackBefore(Image, "OutputDebugStringA", 1,
                               AntiDbgLogFuncOccurrence);
      AntiDbgAddCallbackBefore(Image, "OutputDebugStringW", 1,
                               AntiDbgLogFuncOccurrence);
    }
  }
  if (util::iequals(dllName, "user32")) {
    AntiDbgAddCallbackBefore(Image, "BlockInput", 1, AntiDbg_BlockInput);
    AntiDbgAddCallbackBefore(Image, "SwitchDesktop", 1,
                             AntiDbgLogFuncOccurrence);
    ...
    ////////////////////////////////////
    if (m_Settings.antidebug >= ANTIDEBUG_DEEP) {
      AntiDbgAddCallbackBefore(Image, "GetWindowTextA", 3,
                               AntiDbgLogFuncOccurrence);
      AntiDbgAddCallbackBefore(Image, "GetWindowTextW", 3,
                               AntiDbgLogFuncOccurrence);
    }
  }
