...
VOID ImageLoad(IMG Image, VOID *v)
{
    ...
    if (util::iequals(dllName, "Kernel32")) {
        RTN RTN_VirtualQuery = RTN_FindByName(Image, "VirtualQuery");
        ADDRINT Address = RTN_Address(RTN_VirtualQuery);
        if (RTN_Valid(RTN_VirtualQuery)) {
            gVirtualQuery = (_VirtualQuery)Address;
            Log(true,"Find VirtualQuery %p\n", gVirtualQuery);
        }
